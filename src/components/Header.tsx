...
export default function Header() {
  ...
  return (
    <header>
      ...
      {/* Navbar */}
      <nav
        className={`sticky top-0 z-30 bg-white transition-shadow ${scrolled ? "shadow-lg" : "shadow"}`}
      >
        ...
        {menuOpen && (
          <ul className="md:hidden w-full bg-white border-t border-gray-200 font-montserrat uppercase text-gray-800">
            {[
              "HOME",
              "GROUP & PRIVATE OBEDIENCE",
              "PUPPY TRAINING",
              ...
              <li key={label} className="border-b border-gray-200" onClick={closeMenu}>
                <Link href={`#${label.toLowerCase().replace(/[^a-z]/g, "")}`} className="block px-4 py-3">
                  {label}
                </Link>
              </li>
            ))}
